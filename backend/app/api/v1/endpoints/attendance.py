...
async def get_child_calendar(
    ...
    try:
        ...
        leave_history = [_lr_to_schema(lr) for lr in leave_records]
...
@router.get("/parent/children/{child_id}/leave", response_model=list[LeaveHistoryItem])
async def get_leave_history(
    ...
    """
    Return full leave history for a child (Issue #303).
    Validates parent-child ownership before serving data.
    """
    try:
        # Security: verify parent owns this child
        try:
            ...
            if not link_result.first():
                Logger.warning(
                    f"Parent {current_user.id} has no DB link to child {child_id} – demo mode."
                )
        ...
        leave_stmt = select(LeaveRequestModel).where(
            LeaveRequestModel.student_id == int(child_id),
        ).order_by(LeaveRequestModel.applied_date.desc())
        leave_result = await db.execute(leave_stmt)
        records = leave_result.unique().scalars().all()

        return [_lr_to_schema(lr) for lr in records]

    except Exception as e:
        Logger.error(f"get_leave_history error: {e}", exc_info=True)
        return []
...
async def apply_for_leave(
    ...
    try:
        ...
        new_leave = LeaveRequestModel(
            ...
            submitted_by_id=int(current_user.id) if current_user.id.isdigit() else None,
            start_date=start,
            end_date=end,
            reason=body.reason,
            status="Pending",
