"""
Class section repository for handling database operations
related to class sections.
"""
...
class ClassRepository:
    ...
    async def get_by_id(
        self, class_id: int
    ) -> ClassSectionModel | None:
        """
        Fetch a class section by ID.

        Uses selectinload to eagerly load the subjects
        ...
        result = await self.db.execute(
            ...
            .options(
                selectinload(ClassSectionModel.subjects)
            )
    ...
    async def save(
        self, class_obj: ClassSectionModel
    ) -> ClassSectionModel:
        """
